...
def insert_old_blockhash(b_num):
    ...
    if web3.toHex(contract.functions.get_blockhash(b_num + 1).call()) != ZERO:
        child_header = web3.toHex(rlp.encode(to_blockheader(web3.eth.getBlock(b_num + 1))))
        print(child_header)
        ...
        for i in range(0, MANY):
            ...
            try:
                if web3.toHex(contract.functions.get_blockhash(b_num - i).call()) == ZERO:
                    ...
                    return ZERO if tx_hash == ZERO else wait_confirmation_of_last_tx(tx_hash, b_num, MANY)
            except ValueError as e:
                print(e)
